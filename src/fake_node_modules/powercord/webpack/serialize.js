...
function deserialize (obj, seen = new WeakMap()) {
  ...
  if (obj.$$type === '$pointer') {
    ...
    const { ptr } = obj;
    const cached = pointerCache[ptr]?.deref();
    if (cached) {
      return cached;
    }
    ...
    pointerCache[ptr] = new WeakRef(proxy);
    return proxy;
  }
  ...
  if (obj.$$type === '$function') {
    if (!functionWrappers[obj.ref]) {
      ...
      functionWrappers[obj.ref].toString = () => obj.str;
    }
  ...
  if (ptr !== null) {
    pointerCache[ptr] = new WeakRef(res);
  }
...
} else if (EXPOSE_DEBUGGING_HELPER) {
  window.__$$SerializeDebuggerHelper = Object.create(null);
